...
import { MapPin, BookOpen, Award, ArrowLeft, DollarSign } from 'lucide-react';
...
const SchoolDetailPage = () => {
  ...
  const school = mockSchoolsData[schoolId];
        ...
        <Card className="overflow-hidden glass-card shadow-xl">
          <div className="relative h-64 md:h-80 bg-gradient-to-r from-primary to-accent">
            <img  
             ...
             src="https://images.unsplash.com/photo-1671706462783-33bbff80d325" />
              ...
              <section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-muted-foreground">
                  <div className="flex items-center">
                    ...
                    <span><strong>Địa chỉ:</strong> {school.location}</span>
                  </div>
                  <div className="flex items-center">
                    <BookOpen className="h-5 w-5 mr-2 text-primary" />
                    <span><strong>Năm thành lập:</strong> {school.established}</span>
                  </div>
                  <div className="flex items-center">
                    ...
                    <span><strong>Website:</strong> <a href={school.website} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">{school.website}</a></span>
                  </div>
                   <div className="flex items-center">
                    ...
                    <span><strong>Email:</strong> <a href={`mailto:${school.contactEmail}`} className="text-primary hover:underline">{school.contactEmail}</a></span>
                  </div>
                </div>
              </section>
            </div>
      ...
      >
        <Card className="glass-card shadow-lg">
          <CardHeader>
            ...
            <CardDescription>Khám phá các cơ hội học bổng từ {school.name}.</CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  <Card key={scholarship.id} className="hover:shadow-lg transition-shadow duration-300 bg-background/70">
                    ...
                    <CardContent className="space-y-2">
                      ...
                      <p className="text-sm text-muted-foreground flex items-center">
                        ...
                        <strong>Lĩnh vực:</strong> {scholarship.field}
                      </p>
                      <p className="text-sm text-destructive">
                        <strong>Hạn nộp:</strong> {new Date(scholarship.deadline).toLocaleDateString('vi-VN')}
                      </p>
                    </CardContent>
                    <div className="p-6 pt-0">
                       <Button asChild className="w-full">
                        <Link to={`/scholarships/${scholarship.id}`}>Xem Chi Tiết</Link>
                      </Button>
                    </div>
                  </Card>
                ))}
